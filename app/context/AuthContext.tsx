...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const completeLogin = async (password: string, contextData: ContextDataType): Promise<void> => {
    ...
    try {
      ...
      // Use direct value instead of relying on context sync
      if (userData.role === "student" && userData.hasAccess) {
        console.log("Redirecting to AttendanceAccessScreen")
        router.replace("/AttendanceAccessScreen")
      } else {
